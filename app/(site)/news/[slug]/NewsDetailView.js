...
function ActionRow({ liked, bookmarked, copied, onLike, onBookmark, onShare }) {
  return (
    <>
      <ActionButton
        label={liked ? "Remove like" : "Like this update"}
        pressed={liked}
        active={liked}
        onClick={onLike}
      >
      ...
      <ActionButton
        label={bookmarked ? "Remove bookmark" : "Bookmark this update"}
        pressed={bookmarked}
        active={bookmarked}
        onClick={onBookmark}
      >
      ...
      <ActionButton
        label={copied ? "Link copied" : "Share this update"}
        active={copied}
        onClick={onShare}
      >
...
export default function NewsDetailView({ news, relatedNews }) {
  ...
  const readingTime = useMemo(() => calculateReadingTime(news.body), [news.body]);
  const headings = useMemo(() => extractHeadings(news.body), [news.body]);
  ...
  const heroRatio = news.mainImage?.aspectRatio || 16 / 9;
    ...
    <div className="min-h-screen bg-white pb-24 sm:pb-8">
      <div className="page-container pt-24 pb-14 sm:pt-28 md:pt-32">
        ...
        <div className="lg:grid lg:grid-cols-12 lg:gap-12 xl:gap-16">
          ...
          <article className="w-full min-w-0 lg:col-span-8">
              ...
              <div className="mt-7 flex flex-wrap items-center justify-between gap-4 border-y border-neutral-200 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="leading-tight">
                    <p className="text-xs font-semibold text-neutral-900">SUCF UNEC</p>
            ...
            {news.mainImage && (
              ...
              >
                ...
                <Image
                  src={urlFor(news.mainImage).width(HERO_WIDTH).url()}
                  alt={news.title}
                  width={HERO_WIDTH}
                  height={Math.round(HERO_WIDTH / heroRatio)}
                  sizes="(max-width: 1024px) 100vw, 720px"
                  placeholder={news.mainImage.lqip ? "blur" : "empty"}
                  blurDataURL={news.mainImage.lqip}
                  priority
            ...
            <div className="mt-14 rounded-2xl border border-neutral-200 bg-neutral-50 px-6 py-8 sm:px-8">
              ...
              <h2 className="mt-2 text-lg font-semibold tracking-tight text-neutral-900 sm:text-xl">
                Every SUCF UNEC announcement in one place
              </h2>
              <p className="mt-2 max-w-md text-sm leading-relaxed text-neutral-600">
                Programmes, meetings and fellowship updates as they are published.
              </p>
              <Link
                href="/news"
          ...
          {/* Sticky rail — table of contents, publisher card, share actions */}
            ...
            <div className="sticky top-28 space-y-6">
              ...
              <div className="rounded-2xl border border-neutral-200 p-5">
                <div className="flex items-center gap-3">
                  ...
                  <div className="leading-tight">
                    <p className="text-xs font-semibold text-neutral-900">SUCF UNEC</p>
                    <p className="mt-0.5 text-xs text-neutral-600">Newsroom</p>
                  </div>
                </div>
                ...
                <motion.article key={item._id} variants={staggerItem}>
                  <Link href={`/news/${item.slug}`} className="group block">
                    ...
                    <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-emerald-700">
                      {item.publishedAt ? formatDate(item.publishedAt) : "News"}
                    </p>
