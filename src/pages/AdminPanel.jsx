                ...
                <AnimatePresence mode='wait'>
                    {activeTab === 'gallery' ? (
                        ...
                        <motion.div key="articles" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-12">
                            <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100">
                                <h3 className="text-xl font-bold text-emerald-900 mb-6 flex items-center gap-2"><FiPlus className="text-emerald-500" /> Write New Article</h3>
                                <form onSubmit={handleArticleSubmit} className="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <textarea required rows="6" value={articleForm.content} onChange={(e) => setArticleForm({ ...articleForm, content: e.target.value })} className="w-full bg-gray-50 border border-transparent focus:bg-white focus:border-emerald-500 rounded-xl py-3 px-4 outline-none transition-all resize-none"></textarea>
                                    </div>
                                    <div className="flex justify-end"><button disabled={uploading} className="bg-emerald-900 text-white px-10 py-3 rounded-xl font-bold hover:bg-emerald-800 transition-all shadow-lg shadow-emerald-900/20 flex items-center gap-2"><FiSave /> Publish Article</button></div>
                                </form>
                            </div>
                            <div className="space-y-4">
                                {articles.map((article) => (
                                    <div key={article.id} className="bg-white p-6 rounded-2xl border border-gray-100 flex justify-between items-center group hover:shadow-md transition-all">
                                        ...
                                        <button onClick={() => handleDeleteArticle(article.id)} className="p-3 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all"><FiTrash2 /></button>
                                    </div>
                                ))}
                            </div>
                        </motion.div>
